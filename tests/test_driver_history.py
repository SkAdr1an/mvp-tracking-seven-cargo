...
def test_driver_with_and_without_trips_and_incomplete_legacy_record(history):
    ...
    assert service.backfill_internal_trips() == 1
    listed = service.drivers()
    assert listed["total"] == 1
    assert listed["items"][0]["total_trips"] == 1
    with repository.connect() as connection:
        now = datetime.now(timezone.utc).isoformat()
        connection.execute("INSERT INTO driver_profiles(id,name,created_at,updated_at) VALUES('drv-empty','Sem Viagens',?,?)", (now, now))
...
def test_same_names_remain_distinct_by_internal_id_and_cpf(history):
    ...
    now = datetime.now(timezone.utc).isoformat()
    with repository.connect() as connection:
        connection.execute("INSERT INTO driver_profiles VALUES('drv-a','11111111111','José Igual',NULL,?,?)", (now, now))
        connection.execute("INSERT INTO driver_profiles VALUES('drv-b','22222222222','José Igual',NULL,?,?)", (now, now))
    assert service.profile("11111111111")["id"] == "drv-a"
    assert service.profile("drv-b")["cpf"] == "22222222222"
...
def test_filters_pagination_sensitive_pdf_and_migration_rollback(history, tmp_path: Path):
    ...
    migrated = OperationsRepository(migration_db)
    down = Path("migrations/009_driver_history_evaluations.down.sql").read_text(encoding="utf-8")
