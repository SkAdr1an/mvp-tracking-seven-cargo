...
class AuditService:
    ...
    @staticmethod
    def _encode_cursor(occurred_at: str, identifier: str) -> str:
        payload = json.dumps([occurred_at, identifier], separators=(",", ":")).encode("utf-8")
    ...
    def query(self, *, actor_user_id: str | None = None, actor_query: str | None = None,
        ...
        if actor_query and actor_query.strip():
            value = f"%{actor_query.strip().casefold()}%"
            clauses.append("(LOWER(actor_username_snapshot) LIKE ? OR LOWER(actor_display_name_snapshot) LIKE ?)")
        ...
        if cursor:
            ...
            if decoded:
                occurred_at, identifier = decoded
                clauses.append("(occurred_at<? OR (occurred_at=? AND id<?))")
                params.extend((occurred_at, occurred_at, identifier))
        if operational_only:
        ...
        for row in rows:
            ...
            events.append(item)
        next_cursor = self._encode_cursor(events[-1]["occurred_at"], events[-1]["id"]) if has_more and events else None
