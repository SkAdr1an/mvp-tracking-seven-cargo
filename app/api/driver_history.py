...
def service() -> DriverHistoryService:
    return DriverHistoryService(trip_operations_service.repository)
...
def list_drivers(search: str = "", page: int = Query(1, ge=1), page_size: int = Query(25, ge=1, le=100),
    ...
    history = service()
    history.backfill_internal_trips()
    return history.drivers(search=search, page=page, page_size=page_size)
