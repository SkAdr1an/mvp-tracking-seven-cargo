...
export function PortalAlerts({ data, degraded }: { data?: PortalAlertsResponse; degraded: boolean }) {
  const alerts = [...(data?.alerts || [])].sort((left, right) =>
    (left.distance_km ?? Number.POSITIVE_INFINITY) - (right.distance_km ?? Number.POSITIVE_INFINITY))
  ...
  return <section className="public-card public-alerts" aria-labelledby="portal-alerts-title">
    ...
    {alerts.length === 0 ? <p className="public-alerts-empty">Nenhum alerta relevante próximo no momento.</p> : <div className="public-alert-list">{alerts.map((alert) => <article key={alert.id} className={`public-alert public-alert--${alert.severity.toLowerCase()}`}><i>{alertIcon(alert.type)}</i><div><div className="public-alert-heading"><strong>{alertTitle(alert.type)}</strong><span>{alert.distance_km != null ? `A aproximadamente ${formatDistance(alert.distance_km)} à frente` : 'Distância indisponível'}</span></div><p><b>Gravidade: {severityLabel(alert.severity)}.</b> {alert.description}{alert.delay_minutes != null ? ` Acréscimo estimado de ${Math.round(alert.delay_minutes)} minutos.` : ''}</p><small>{alert.reference || 'Referência indisponível'} · {locationAgeLabel(alert.updated_at)}</small><em>{alert.guidance}</em>{isDemo(alert.source) && <mark>Dado fictício de demonstração</mark>}</div></article>)}</div>}
...
function alertTitle(type: string) { return ({ TRANSITO_LENTO: 'Trânsito lento', ACIDENTE_BLOQUEIO: 'Acidente ou bloqueio', CHUVA_LEVE: 'Chuva leve', CHUVA_FORTE: 'Chuva forte ou temporal', CLIMA_NORMAL: 'Condição climática normal', DESVIO_CONFIRMADO: 'Desvio confirmado', DESTINO: 'Aproximação do destino', ORIGEM: 'Aproximação da origem' } as Record<string, string>)[type] || 'Obstáculo à frente' }
function formatDistance(km: number) { return km < 1 ? `${Math.round(km * 1000)} m` : `${km.toLocaleString('pt-BR', { maximumFractionDigits: 1 })} km` }
function severityLabel(value: string) { return value === 'CRITICO' ? 'Crítica' : value === 'ATENCAO' ? 'Atenção' : 'Informativa' }
function isDemo(source: string) { return source.toLocaleLowerCase('pt-BR').includes('fictício') || source.toLocaleLowerCase('pt-BR').includes('demonstração') }
